  ...
  if (err) {
    ...
    describe("GET /grids/rg09_temperature", function () {
      it("grids with data filter should return correct units, as a list when compressed", async function () {
        ...
        expect(response.body[0].units).to.deep.equal(["degree celcius (ITS-90)"])
      });
    });
    ...
    describe("GET /grids/rg09_temperature", function () {
      it("grids with data=all and presRange filters should return correct data", async function () {
        const response = await request.get("/grids/rg09_temperature?id=20040115000000_20.5_-64.5&data=all&presRange=5,100").set({'x-argokey': 'developer'});
    ...
    describe("GET /grids/rg09_temperature", function () {
      it("grids with data=all and presRange filters should report levels, post filtering, on the data document", async function () {
        const response = await request.get("/grids/rg09_temperature?id=20040115000000_29.5_-64.5&data=all&presRange=5,100").set({'x-argokey': 'developer'});
    ...
    describe("GET /grids/rg09_temperature", function () {
      it("fetch gridded data with pressure bracket", async function () {
        const response = await request.get("/grids/rg09_temperature?id=20040115000000_20.5_-64.5&presRange=10,100&data=rg09_temperature&compression=array").set({'x-argokey': 'developer'});
    ...
    describe("GET /grids/rg09_temperature", function () {
      it("fetch gridded data in overlap region between two polygons", async function () {
        const response = await request.get("/grids/rg09_temperature?multipolygon=[[[22,-65],[22,-64],[26,-64],[26,-65],[22,-65]],[[24,-65],[24,-64],[30,-64],[30,-65],[24,-65]]]&startDate=2000-01-01T00:00:00Z&endDate=2020-01-01T00:00:00Z").set({'x-argokey': 'developer'});
    ...
    describe("GET /grids/rg09_temperature", function () {
      it("should return appropriate minimal representation of this measurement", async function () {
        const response = await request.get("/grids/rg09_temperature?id=20040115000000_20.5_-64.5&compression=minimal").set({'x-argokey': 'developer'});
