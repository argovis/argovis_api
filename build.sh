...
docker container run --rm -v ${PWD}:/local swaggerapi/swagger-codegen-cli-v3 generate -i /local/bsose-spec.json -l nodejs-server -o /local/nodejs-server
mv nodejs-server/api/openapi.yaml nodejs-server/api/openapi.bsose.yaml